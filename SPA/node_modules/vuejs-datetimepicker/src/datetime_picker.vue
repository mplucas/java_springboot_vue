<template>
  <div :style='{ width: width }' class="datetime-picker" v-on:click='calendarClicked($event)' v-on:blur='toggleCal'>
    <div>
      <input type='text' class="form-control" :readonly="readonly" id='tj-datetime-input' :required="required"
        :value="valueString" :name='name' v-on:click='toggleCal' autocomplete='off' />
      <div class='calender-div' :class='{ noDisplay: hideCal }'>
        <div :class='{ noDisplay: hideDate }'>
          ...
          <div>
            <div class="week" v-for="(week, weekIndex) in weeks" :key="weekIndex">
              <span class="port" v-for="(day, dayIndex) in week" :key="dayIndex"
        ...
        <div class='time-picker' :class='{ noDisplay: hideTime }'>
          <div class='hour-selector'>
            <div v-on:click='showHourSelector' id='j-hour'>{{ periodStyle === 12 && hour > 12 ? hour - 12 : hour }}
            </div>
            <div class='scroll-hider' ref='hourScrollerWrapper' :class='{ showSelector: hourSelectorVisible }'>
              <ul ref='hourScroller'>
                <li v-for="(h, index) in hours" :key="index" :class='{ active: index === hourIndex }'
                  v-on:click='setHour(index, true)'>{{ h }}</li>
              </ul>
            </div>
          </div>
          ...
          <div class='minute-selector'>
            ...
            <div class='scroll-hider' ref='minuteScrollerWrapper' :class='{ showSelector: minuteSelectorVisible }'>
              <ul ref='minuteScroller'>
                <li v-for="(m, index) in minutes" :key="index" :class='{ active: index === minuteIndex }'
                  v-on:click='setMinute(index, true)'>{{ m }}</li>
              </ul>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    setDate() {
      ...
      d = d.replace('s', '00')
      this.valueString = d
      this.$emit('input', this.makeDateObject(d))
      this.hideCal = true
    },
    /**
  ...
  watch: {
    valueString(newVal, oldVal) {
      if (newVal) {
        ...
        try {
          this.timeStamp = this.makeDateObject(this.valueString)
        } catch (e) {
